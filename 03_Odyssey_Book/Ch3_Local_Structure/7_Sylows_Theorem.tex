...
\begin{solution}
    ...
    \begin{enumerate}
        ...
        \item[(e)] Let $|G|=180=2^2\cdot 3^2\cdot 5$. Let $Q$ be a Sylow $3$-subgroup and let $1\neq x\in Q$. Since $|Q|=9$, the group $Q$ is abelian, so $Q\leq C_G(x)$. Thus $|C_G(x)|$ is divisible by $9$. The possibilities are $9$, $18$, $36$, $90$, and $180$. If $|C_G(x)|=180$, then $\langle x\rangle\trianglelefteq G$. If $|C_G(x)|=90$, then $C_G(x)$ has index $2$ and is normal in $G$. If $|C_G(x)|=36$, then the coset action gives an embedding $G\hookrightarrow S_5$, impossible because $180\nmid 120$. Hence
        \[
            |C_G(x):Q|\leq 2.
        \]
