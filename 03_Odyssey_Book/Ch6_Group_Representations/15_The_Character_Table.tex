...
\begin{itemize}
    \item Any character of $G$ is a $\Z$-linear combination of the $r$ irreducible characters $\chi_1, \ldots, \chi_r$, where $r$ is the number of conjugacy classes of $G$ (Theorem~\ref{thm:14.3}).
...
\begin{corollary}
\label{cor:15.3}
If $\alpha$ is a character of $G$ and $n \in \{1, 2, 3\}$, then $(\alpha, \alpha) = n$ iff $\alpha$ is a sum of $n$ irreducible characters.
\end{corollary}
\begin{proof}
\end{proof}
...
\begin{example}
\label{ex:15.eg.6}
Consider $S_3$. The conjugacy classes of $S_3$ are $\{1\}$, $\{(1\,2), (1\,3), (2\,3)\}$, and $\{(1\,2\,3), (1\,3\,2)\}$. (This follows from Proposition~\ref{prop:1.10}.) We have $S_3/A_3 \cong \Z_2$, so $S_3$ has two linear characters, which are lifted from the characters of $\Z_2$ as in Lemma~\ref{lem:15.6}. Letting these characters be $\chi_1$ and $\chi_2$, we have
\[
    \begin{array}{c|ccc}
        & 1 & 3 & 2 \\
        & 1 & (1\,2) & (1\,2\,3) \\
