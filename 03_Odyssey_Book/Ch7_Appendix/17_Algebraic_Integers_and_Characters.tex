...
\begin{proof}
    ...
    \]
    with each $\lvert C_i\rvert$ divisible by $pq$, a contradiction since $pq$ divides $\lvert G\rvert$. Therefore $G$ has a non-trivial conjugacy class whose order is not divisible by both $p$ and $q$. Since every conjugacy class size divides $\lvert G\rvert$, this class has prime power order. Theorem~\ref{thm:17.7} then implies that $G$ is not simple, contradicting our assumption. Thus the non-abelian simple case cannot occur, and $G$ is solvable.
\end{proof}
